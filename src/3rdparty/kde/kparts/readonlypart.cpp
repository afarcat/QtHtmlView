...
void ReadOnlyPart::setUrl(const QUrl &url)
{
    ...
    if (d->m_url != url) {
      //AFA
      QUrl prevUrl = d->m_url;
      d->m_url = url;
      emit urlChanged(url, prevUrl);
    }
}
...
bool ReadOnlyPart::openUrl(const QUrl &url)
{
    ...
    if (d->m_url.isLocalFile()) {
    ...
    } else if (/*AFA KProtocolInfo::protocolClass*/(url.scheme()) == QLatin1String(":local")) {
        ...
        //AFA d->m_statJob = KIO::mostLocalUrl(d->m_url, flags);
        //AFA KJobWidgets::setWindow(d->m_statJob, widget());
        //AFA connect(d->m_statJob, SIGNAL(result(KJob*)), this, SLOT(_k_slotStatJobFinished(KJob*)));
        return true;
    } else {
        d->openRemoteFile();
        return true;
