...
//AFA #include <kio/jobclasses.h>
//AFA #include <kio/scheduler.h>
//AFA #include <kcharsets.h>
...
QTextCodec *CachedObject::codecForBuffer(const QString &charset, const QByteArray &buffer) const
{
    ...
    // Link or @charset
    if (!charset.isEmpty()) {
        QTextCodec *c = /*AFA KCharsets::charsets()->*/QTextCodec::codecForName(charset.toUtf8());
        if (c->mibEnum() == 11)  {
...
void CachedCSSStyleSheet::data(QBuffer &buffer, bool eof)
{
    ...
    if (!m_charset.isEmpty()) {
        c = /*AFA KCharsets::charsets()->*/QTextCodec::codecForName(m_charset.toUtf8());
        if (c->mibEnum() == 11) {
            c = QTextCodec::codecForName("iso8859-8-i");
        }
    } else {
...
QPixmap CachedImage::pixmap() const
{
    ...
    //AFA: QWidget::paintEngine: Should no longer be called
    if (false/*AFA i->hasAlpha() && QApplication::desktop()->paintEngine() &&
                                    ...
                                    bool doRedirectCheck, bool isImg)
{
    ...
    if (doRedirectCheck && doc) {
        if (isImg && part && part->forcePermitLocalImages() && fullURL.scheme() == "file") {
            ...
            //AFA return KUrlAuthorized::authorizeUrlAction("redirect", doc->URL(), fullURL);
        }
    }
...
void Loader::scheduleRequest(Request *req)
{
    ...
    KIO::Job *job = KIO::get(u, KIO::NoReload, KIO::HideProgressInfo /*no GUI*/);
    ...
    if (req->m_docLoader) {
        ...
        if (part) {
            ...
            //AFA if (part->widget()) {
            //AFA     KJobWidgets::setWindow(job, part->widget()->topLevelWidget());
            //AFA }
        }
    }
    ...
    //AFA KIO::Scheduler::setJobPriority(job, req->priority);
...
void Cache::init()
{
    ...
    if (!brokenPixmap) {
        //AFA brokenPixmap = new QPixmap(KHTMLGlobal::iconLoader()->loadIcon("image-missing", KIconLoader::Desktop, 16, KIconLoader::DisabledState));
        brokenPixmap = new QPixmap(":/resources/images/image-missing.png");
    }
