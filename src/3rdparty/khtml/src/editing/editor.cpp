...
void Editor::copy()
{
    //AFA static_cast<KHTMLPartBrowserExtension *>(m_part->browserExtension())->copy();
}

void Editor::cut()
{
    // ###
    //AFA static_cast<KHTMLPartBrowserExtension *>(m_part->browserExtension())->cut();
}
...
void Editor::print()
{
    //AFA static_cast<KHTMLPartBrowserExtension *>(m_part->browserExtension())->print();
}
...
CSSStyleDeclarationImpl *Editor::selectionComputedStyle(NodeImpl *&nodeToRemove) const
{
    ...
    NodeImpl *start, *end;
    long so, eo;
    ctx->m_selection.getRange(start, so, end, eo);
    Position pos(start, so);
#endif
    assert(pos.notEmpty());
