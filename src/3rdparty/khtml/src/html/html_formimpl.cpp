    ...
    : HTMLElementImpl(doc)
{
    ...
    //AFA m_boundary = "----------" + KRandom::randomString(42 + 13);
...
QByteArray HTMLFormElementImpl::formData(bool &ok)
{
    ...
    if (str == "UNKNOWN") {
        ...
        if (view && view->part()) {
            //AFA codec = KCharsets::charsets()->codecForName(view->part()->encoding(), codecOk);
            codec = QTextCodec::codecForName(view->part()->encoding().toUtf8());
            codecOk = (codec != nullptr) ? true : false;
        }
    }
    ...
    if (!codecOk) {
        ...
        for (QStringList::ConstIterator it = charsets.constBegin(); it != charsets.constEnd(); ++it) {
            //AFA codec = KCharsets::charsets()->codecForName((*it), codecOk);
            codec = QTextCodec::codecForName((*it).toUtf8());
            codecOk = (codec != nullptr) ? true : false;
    ...
    if (!codecOk) {
        ...
        //AFA codec = KCharsets::charsets()->codecForName("UTF-8", codecOk);
