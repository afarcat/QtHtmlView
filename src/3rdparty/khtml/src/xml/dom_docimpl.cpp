...
NodeImpl *DocumentImpl::importNode(NodeImpl *importedNode, bool deep, int &exceptioncode)
{
    ...
    if (deep && result) {
        //AFA
        for (NodeImpl *n = importedNode->firstChild(); n != nullptr; n = n->nextSibling()) {
            result->appendChild(importNode(n, true, exceptioncode), exceptioncode);
        }
    }
...
void DocumentImpl::setTitle(const DOMString &_title)
{
    ...
    if (view() && !view()->part()->parentPart()) {
        ...
        //AFA emit view()->part()->setWindowCaption(titleStr);
    }
}
...
bool DocumentImpl::isURLAllowed(const QString &url) const
{
    ...
    //AFA if (newURL.scheme() != "javascript" && !KUrlAuthorized::authorizeUrlAction("redirect", thisPart->url(), newURL)) {
    //AFA     return false;
    //AFA }
...
void DocumentImpl::error(int err, const QString &text)
{
    ...
    if (err != 0) {
        //AFA evt->setMessage(KIO::buildErrorString(err, text));
    } else {
        evt->setMessage(text);
    }
    evt->ref();
