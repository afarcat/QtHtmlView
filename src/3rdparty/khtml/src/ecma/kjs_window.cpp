...
//AFA #include <kbookmarkmanager.h>
//AFA #include <kbookmarkdialog.h>
#endif
#include <assert.h>
#include <QStyle>
...
JSValue *Window::getValueProperty(ExecState *exec, int token)
{
    ...
    case SideBar:
        //AFA return new MozillaSidebarExtension(exec, part);
        return jsUndefined();
    case _Navigator:
    case ClientInformation: {
        // Store the navigator in the object so we get the same one each time.
        //AFA JSValue *nav(new Navigator(exec, part));
        //AFA const_cast<Window *>(this)->put(exec, "navigator", nav, DontDelete | ReadOnly | Internal);
        //AFA const_cast<Window *>(this)->put(exec, "clientInformation", nav, DontDelete | ReadOnly | Internal);
        //AFA return nav;
        return jsUndefined();
    }
...
JSValue *WindowFunc::callAsFunction(ExecState *exec, JSObject *thisObj, const List &args)
{
    ...
    case Window::Alert: {
        ...
        //AFA KMessageBox::error(widget, Qt::convertFromPlainText(str), caption);
        return jsUndefined();
    }
    case Window::Confirm: {
        TimerPauser pause(exec);
        ...
        return jsBoolean(false); //AFA jsBoolean((KMessageBox::warningYesNo(widget, Qt::convertFromPlainText(str), caption,
               //AFA            KStandardGuiItem::ok(), KStandardGuiItem::cancel()) == KMessageBox::Yes));
    }
    case Window::Prompt: {
        TimerPauser pause(exec);
#ifndef KONQ_EMBEDDED
