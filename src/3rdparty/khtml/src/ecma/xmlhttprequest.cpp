...
void XMLHttpRequest::send(const QString &_body, int &ec)
{
    ...
    if (m_method == QLatin1String("POST") || havePayload) {
        ...
        //AFA job = KIO::storedHttpPost(buf, url, KIO::HideProgressInfo);
    } else {
        //AFA job = KIO::storedGet(url, KIO::NoReload, KIO::HideProgressInfo);
    }
    ...
    if (!async) {
        ...
        if (job->exec()) {
            //AFA data = job->data();
            //AFA finalURL = job->redirectUrl().isEmpty() ? job->url() : job->redirectUrl();
...
#else
    //AFA KIO::Scheduler::setJobPriority(job, 1);
#endif
}
