 ...
 *
 */
class KHTML_EXPORT QHTMLPart : public KParts::ReadOnlyPart
{
    Q_OBJECT
    ...
    friend class QHTMLPartBrowserExtension;
    ...
    friend class khtml::QHTMLPartAccessor;
    friend class QHTMLPartIface;
    friend class QHTMLPartFunction;
...
public:
    ...
    */
     ...
     *
     * KHTML basically consists of two objects: The QHTMLPart itself,
     * holding the document data (DOM document), and the QHTMLView,
     * derived from QScrollArea, in which the document content is
     * rendered in. You can specify two different parent objects for a
     * QHTMLPart, one parent for the QHTMLPart document and one parent
     * for the QHTMLView. If the second @p parent argument is 0L, then
     ...
     * Constructs a new QHTMLPart.
     *
     * This constructor is useful if you wish to subclass QHTMLView.
     * If the @p view passed  as first argument to the constructor was built with a
     * null QHTMLPart pointer, then the newly created QHTMLPart will be assigned as the view's part.
     *
     * Therefore, you might either initialize the view as part of the initialization list of
     * your derived QHTMLPart class constructor:
     * \code
     *   MyQHTMLPart() : QHTMLPart( new MyQHTMLView( this ), ...
     * \endcode
     * Or separately build the QHTMLView beforehand:
     * \code
     *   QHTMLView * v = QHTMLView( 0L, parentWidget());
...
public slots:
     ...
     */
    DOM::HTMLDocumentImpl *htmlDocument() const;
...
public:
    ...
    /**
     * Paints the HTML page to a QPainter. See QHTMLView::paint for details
     */
...
public:
     ...
     */
    enum FindOptions {
        FindLinksOnly   = 1,//AFA  * KFind::MinimumUserOption,
        FindNoPopups    = 2 //AFA * KFind::MinimumUserOption
     ...
     */
    void findText(const QString &str, long options, QWidget *parent = nullptr/*AFA,
    ...
    /**
     * Saves the QHTMLPart's complete state (including child frame
    ...
    /**
     * Restores the QHTMLPart's previously saved state (including
     ...
     * @internal
     */
    const QHTMLSettings *settings() const;

    /**
     * Returns a pointer to the parent QHTMLPart if the part is a frame
     * in an HTML frameset.
     *
     *  Returns 0L otherwise.
     */
    // ### KDE5 make const
    QHTMLPart *parentPart();
     ...
     */
    QHTMLPart *findFrame(const QString &f);
     ...
     */
    QHTMLPart *findFrameParent(KParts::ReadOnlyPart *callingPart, const QString &f, khtml::ChildFrame **childFrame = nullptr);
     ...
     */
    void setSuppressedPopupIndicator(bool enable, QHTMLPart *originPart = nullptr);
...
protected Q_SLOTS:
     ...
     * but make sure to call QHTMLPart::slotFinished() too.
     */
...
private Q_SLOTS:
    ...
    */
     ...
     * @internal
     */
    //AFA void slotAutomaticDetectionLanguage(KEncodingProber::ProberType scri);
...
private:
    ...
    bool checkFrameAccess(QHTMLPart *callingHtmlPart);
     ...
     */
    QHTMLViewBar *pTopViewBar() const;

    /**
     * Returns a pointer to the bottom view bar.
     */
    QHTMLViewBar *pBottomViewBar() const;
#endif
    ...
    khtml::ChildFrame *recursiveFrameRequest(QHTMLPart *callingHtmlPart, const QUrl &url,
            const KParts::OpenUrlArguments &args, const KParts::BrowserArguments &browserArgs,
            bool callParent = true);

    bool checkLinkSecurity(const QUrl &linkURL, const QString &message = QString(), const QString &button = QString());
    ...
    QHTMLPart *opener();
    long cacheId() const;
    void setOpener(QHTMLPart *_opener);
    bool openedByJS();
