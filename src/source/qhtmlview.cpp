...
//AFA #include "khtml_printsettings.h"

#include "qhtmlpart_p.h"

//AFA #include <kcursor.h>
#include "khtml_debug.h"
//AFA #include <kiconloader.h>
//AFA #include <knotification.h>
//AFA #include <kconfig.h>
...
class QHTMLViewPrivate
{
    friend class QHTMLView;
public:
        ...
        : underMouse(nullptr), underMouseNonShared(nullptr), oldUnderMouse(nullptr)
    {
        ...
        smoothScrollMode = QHTMLView::SSMWhenEfficient;
        ...
        cursorIconType   = QHTMLView::LINK_NORMAL;
    ...
    void reset()
    {
        ...
        // so exceptionally, we need to directly ref/deref QHTMLGlobal to
        // account for this transitory case.
        QHTMLGlobal::ref();
        accessKeysEnabled = QHTMLGlobal::defaultHTMLSettings()->accessKeysEnabled();
        QHTMLGlobal::deref();
...
#else
        QQuickFlickable *flickable = view->flickable();
        if (flickable) {
            contentsX = flickable->contentX();
            contentsY = flickable->contentY();
        }
#endif
    }
...
#else
            it.value()->setPosition(it.value()->position() + QPoint(dx, dy));
#endif
        }
    }
    ...
    QHTMLView::SmoothScrollingMode smoothScrollMode : 3;
            ...
            if (node->isElementNode()) {
                ...
                if (!s.isEmpty()) {
                    ...
                    QToolTip::showText(he->globalPos(),
                                       ...
                                       widget(), region);
#else
                    emit m_part->showToolTip(he->pos(), Qt::convertFromPlainText(s, Qt::WhiteSpaceNormal));
#endif
                    break;
                }
            }
...
#ifdef QT_WIDGETS_LIB
QHTMLView::QHTMLView(QHTMLPart *part, QWidget *parent)
    : QScrollArea(parent), d(new QHTMLViewPrivate(this))
#else
...
QQuickScrollBar *KHTMLView::horizontalScrollBar() const
{
    if (!d->viewport) {
        return nullptr;
    }
...
QQuickScrollBar *KHTMLView::verticalScrollBar() const
{
    if (!d->viewport) {
        return nullptr;
    }
...
void KHTMLView::geometryChanged(const QRectF &newGeometry, const QRectF &oldGeometry)
{
    resizeEvent(nullptr);
}
#endif

void QHTMLView::setPart(QHTMLPart *part)
{
    assert(part && !m_part);
    m_part = part;
}
...
QWidget *QHTMLView::mouseEventsTarget() const
{
    return d->m_mouseEventsTarget;
}

void QHTMLView::setClipHolder(QStack<QRegion> *ch)
{
    d->m_clipHolder = ch;
}

QStack<QRegion> *QHTMLView::clipHolder() const
{
...
#else
    return d->viewport ? d->viewport->contentWidth() : 0;
#endif
}
...
#else
    return d->viewport ? d->viewport->contentHeight() : 0;
#endif
}
...
#else
    if (d->viewport) {
        d->viewport->setContentWidth(w);
        d->viewport->setContentHeight(h);
...
int QHTMLView::visibleWidth() const
{
    ...
    return d->viewport ? d->viewport->width() : 0;
#endif
}
...
int QHTMLView::visibleHeight() const
{
    ...
    return d->viewport ? d->viewport->height() : 0;
#endif
}
...
void QHTMLView::contentsToViewport(int x, int y, int &cx, int &cy) const
{
    QPoint p(x, y);
...
QPoint QHTMLView::viewportToContents(const QPoint &p) const
{
    return QPoint(p.x() + contentsX(), p.y() + contentsY());
}

void QHTMLView::viewportToContents(int x, int y, int &cx, int &cy) const
{
    QPoint p(x, y);
...
void QHTMLView::updateContents(int x, int y, int w, int h)
{
...
void QHTMLView::updateContents(const QRect &r)
{
    updateContents(r.x(), r.y(), r.width(), r.height());
}

void QHTMLView::repaintContents(int x, int y, int w, int h)
{
    applyTransforms(x, y, w, h);
    if (m_kwp->isRedirected()) {
        QPoint off = m_kwp->absolutePos();
...
void QHTMLView::repaintContents(const QRect &r)
{
    repaintContents(r.x(), r.y(), r.width(), r.height());
}

void QHTMLView::applyTransforms(int &x, int &y, int &w, int &h) const
{
    if (d->haveZoom()) {
...
void QHTMLView::revertTransforms(int &x, int &y, int &w, int &h) const
{
    x += contentsX();
    y += contentsY();
    if (d->haveZoom()) {
...
void QHTMLView::revertTransforms(int &x, int &y) const
{
    int dummy = 0;
...
#else
    QCoreApplication::sendPostedEvents(this, QEvent::Paint);
#endif
...
void KHTMLView::paint(QPainter *painter)
{
    ...
    if (!m_part || !m_part->xmlDocImpl() || !m_part->xmlDocImpl()->renderer()) {
        painter->fillRect(ex, ey, ew, eh, qApp->palette().brush(QPalette::Active, QPalette::Base));
        return;
    } else if (d->complete && static_cast<RenderCanvas *>(m_part->xmlDocImpl()->renderer())->needsLayout()) {
        // an external update request happens while we have a layout scheduled
        unscheduleRelayout();
    ...
    if (d->hasFrameset) {
        NodeImpl *body = static_cast<HTMLDocumentImpl *>(m_part->xmlDocImpl())->body();
        if (body && body->renderer() && body->id() == ID_FRAMESET) {
            static_cast<RenderFrameSet *>(body->renderer())->paintFrameSetRules(painter, r);
        } else {
            d->hasFrameset = false;
        }
    }

    khtml::DrawContentsEvent event(painter, ex, ey, ew, eh);
    QCoreApplication::sendEvent(m_part, &event);

    if (d->contentsMoving && !d->smoothScrolling/* && widget()->underMouse()*/) {
        QMouseEvent *tempEvent = new QMouseEvent(QEvent::MouseMove, mapFromGlobal(QCursor::pos()),
                Qt::NoButton, Qt::NoButton, Qt::NoModifier);
        QCoreApplication::postEvent(this, tempEvent);
    }
...
void QHTMLView::setMarginHeight(int h)
{
    // make it update the rendering area when set
    _marginHeight = h;
}

void QHTMLView::layout()
{
    if (m_part && m_part->xmlDocImpl()) {
...
bool QHTMLView::dialogsAllowed()
{
    bool allowed = d->m_dialogsAllowed;
    QHTMLPart *p = m_part->parentPart();
    if (p && p->view()) {
...
int QHTMLView::zoomLevel() const
{
    return d->zoomLevel;
}

void QHTMLView::setSmoothScrollingMode(SmoothScrollingMode m)
{
...
void QHTMLView::setSmoothScrollingModeDefault(SmoothScrollingMode m)
{
...
QHTMLView::SmoothScrollingMode QHTMLView::smoothScrollingMode() const
{
...
void QHTMLView::mousePressEvent(QMouseEvent *_mouse)
{
    if (!m_part->xmlDocImpl()) {
        return;
    }
    ...
    if ((_mouse->button() == Qt::MidButton) &&
        ...
        //AFA KConfigGroup cg(KSharedConfig::openConfig(), "HTML Settings");
        QSettings cg;
        if (cg.value("HTML Settings/ShowMouseScrollIndicator", true).toBool()) {
...
static bool targetOpensNewWindow(QHTMLPart *part, QString target)
{
...
void QHTMLView::mouseMoveEvent(QMouseEvent *_mouse)
{
    ...
    if (r && r->isWidget()) {
        ...
        KHTMLWidget *kw = qobject_cast<QHTMLView *>(rw->widget()) ? dynamic_cast<KHTMLWidget *>(rw->widget()) : nullptr;
    ...
    if (vp && setCursor && (vp->cursor().shape() != c.shape() || c.shape() == Qt::BitmapCursor)) {
    ...
    if (linkCursor != LINK_NORMAL && isVisible() && hasFocus()) {
        ...
        if (linkCursor != d->cursorIconType) {
            ...
            QPixmap icon_pixmap = QHTMLGlobal::iconLoader()->loadIcon(cursorIcon, KIconLoader::Small, 0, KIconLoader::DefaultState, QStringList(), nullptr, true);
...
bool QHTMLView::dispatchKeyEvent(QKeyEvent *_ke)
{
    if (!m_part->xmlDocImpl()) {
        return false;
    }
...
// returns true if event should be swallowed
bool QHTMLView::dispatchKeyEventHelper(QKeyEvent *_ke, bool keypress)
{
...
void QHTMLView::keyPressEvent(QKeyEvent *_ke)
{
    ...
    //AFA-modify
    if (m_part->isCaretMode() && caretKeyPressEvent(_ke)) {
        _ke->accept();
        return;
    }
    ...
    if (_ke->modifiers() & Qt::ShiftModifier)
        ...
        case Qt::Key_Down:
        case Qt::Key_J:
            d->adjustScroller(this, QHTMLViewPrivate::ScrollDown, QHTMLViewPrivate::ScrollUp);
            break;

        case Qt::Key_Up:
        case Qt::Key_K:
            d->adjustScroller(this, QHTMLViewPrivate::ScrollUp, QHTMLViewPrivate::ScrollDown);
            break;

        case Qt::Key_Left:
        case Qt::Key_H:
            d->adjustScroller(this, QHTMLViewPrivate::ScrollLeft, QHTMLViewPrivate::ScrollRight);
            break;

        case Qt::Key_Right:
        case Qt::Key_L:
            d->adjustScroller(this, QHTMLViewPrivate::ScrollRight, QHTMLViewPrivate::ScrollLeft);
            break;
        }
    else
        switch (_ke->key()) {
...
void QHTMLView::keyReleaseEvent(QKeyEvent *_ke)
{
    ...
    if (d->accessKeysEnabled) {
        ...
        if (d->accessKeysPreActivate && !(_ke->modifiers() & Qt::ControlModifier)) {
            displayAccessKeys();
            m_part->setStatusBarText(i18n("Access Keys activated"), QHTMLPart::BarOverrideText);
...
bool QHTMLView::focusNextPrevChild(bool next)
{
    ...
    d->pseudoFocusNode = QHTMLViewPrivate::PFNone;
...
static void handleWidget(QWidget *w, QHTMLView *view, bool recurse = true)
{
    ...
    if (qobject_cast<QHTMLView *>(w)) {
        handleWidget(static_cast<QHTMLView *>(w)->widget(), view, false);
        handleWidget(static_cast<QHTMLView *>(w)->horizontalScrollBar(), view, false);
        handleWidget(static_cast<QHTMLView *>(w)->verticalScrollBar(), view, false);
        return;
    }
...
bool  QHTMLView::viewportEvent(QEvent *e)
{
    switch (e->type()) {
...
static void setInPaintEventFlag(QWidget *w, bool b = true, bool recurse = true)
{
    ...
    if (qobject_cast<QHTMLView *>(w)) {
        setInPaintEventFlag(static_cast<QHTMLView *>(w)->widget(), b, false);
        setInPaintEventFlag(static_cast<QHTMLView *>(w)->horizontalScrollBar(), b, false);
        setInPaintEventFlag(static_cast<QHTMLView *>(w)->verticalScrollBar(), b, false);
        return;
    }
...
#else
                if (w->parentItem() == view && !qobject_cast<QScrollBar *>(w)) {
#endif
...
//                }
            ...
            case QEvent::FocusOut: {
                QPoint dummy;
                QHTMLView *root = m_kwp->rootViewPos(dummy);
                if (!root) {
                    root = this;
                }
#ifdef QT_WIDGETS_LIB
...
bool QHTMLView::widgetEvent(QEvent *e)
{
    switch (e->type()) {
    case QEvent::MouseButtonPress:
    case QEvent::MouseButtonRelease:
...
bool QHTMLView::hasLayoutPending()
{
    return d->layoutTimerId && !d->firstLayoutPending;
}

DOM::NodeImpl *QHTMLView::nodeUnderMouse() const
{
    return d->underMouse;
}

DOM::NodeImpl *QHTMLView::nonSharedNodeUnderMouse() const
{
    return d->underMouseNonShared;
}

bool QHTMLView::scrollTo(const QRect &bounds)
{
...
bool QHTMLView::focusNextPrevNode(bool next)
{
    ...
    if (d->tabMovePending && next != d->lastTabbingDirection) {
        ...
        if (oldFocusNode || d->pseudoFocusNode == QHTMLViewPrivate::PFTop) {
            newFocusNode = doc->nextFocusNode(oldFocusNode);
        }
    } else {
        if (oldFocusNode || d->pseudoFocusNode == QHTMLViewPrivate::PFBottom) {
    ...
    if (targetVisible) {
        ...
        if (newFocusNode) {
            ...
            d->pseudoFocusNode = next ? QHTMLViewPrivate::PFBottom : QHTMLViewPrivate::PFTop;
            return false;
        }
    } else {
        if (!d->tabMovePending) {
...
void QHTMLView::displayAccessKeys()
{
    QVector< QChar > taken;
...
void QHTMLView::displayAccessKeys(QHTMLView *caller, QHTMLView *origview, QVector< QChar > &taken, bool use_fallbacks)
{
    ...
    foreach (KParts::ReadOnlyPart *cur, frames) {
        if (!qobject_cast<QHTMLPart *>(cur)) {
            continue;
        }
        QHTMLPart *part = static_cast< QHTMLPart * >(cur);
...
bool QHTMLView::isScrollingFromMouseWheel() const
{
    return d->scrollingFromWheel != QPoint(-1, -1);
}

void QHTMLView::accessKeysTimeout()
{
    d->accessKeysActivated = false;
    d->accessKeysPreActivate = false;
    m_part->setStatusBarText(QString(), QHTMLPart::BarOverrideText);
    emit hideAccessKeys();
}

// Handling of the HTML accesskey attribute.
bool QHTMLView::handleAccessKey(const QKeyEvent *ev)
{
...
bool QHTMLView::focusNodeWithAccessKey(QChar c, QHTMLView *caller)
{
    DocumentImpl *doc = m_part->xmlDocImpl();
    if (!doc) {
        return false;
...
QMap< ElementImpl *, QChar > QHTMLView::buildFallbackAccessKeys() const
{
...
void QHTMLView::setMediaType(const QString &medium)
{
    m_medium = medium;
}

QString QHTMLView::mediaType() const
{
    return m_medium;
}

bool QHTMLView::pagedMode() const
{
    return d->paged;
}
...
bool QHTMLView::needsFullRepaint() const
{
...
void QHTMLView::print(bool quick)
{
    if (!m_part->xmlDocImpl()) {
        return;
    }
...
void QHTMLView::slotPaletteChanged()
{
    if (!m_part->xmlDocImpl()) {
        return;
    }
...
void QHTMLView::paint(QPainter *p, const QRect &rc, int yOff, bool *more)
{
    if (!m_part->xmlDocImpl()) {
        return;
    }
...
void QHTMLView::render(QPainter *p, const QRect &r, const QPoint &off)
{
#ifdef SPEED_DEBUG
    d->firstRepaintPending = false;
#endif
...
#else
        p->fillRect(clip, qApp->palette().brush(QPalette::Active, QPalette::Base));
#endif
        return;
    }
    QPaintDevice *opd = m_part->xmlDocImpl()->paintDevice();
...
void QHTMLView::setHasStaticBackground(bool partial)
{
    // full static iframe is irreversible for now
    if (d->staticWidget == QHTMLViewPrivate::SBFull && m_kwp->isRedirected()) {
        return;
    }

    d->staticWidget = partial ?
                      QHTMLViewPrivate::SBPartial : QHTMLViewPrivate::SBFull;
}

void QHTMLView::setHasNormalBackground()
{
    // full static iframe is irreversible for now
    if (d->staticWidget == QHTMLViewPrivate::SBFull && m_kwp->isRedirected()) {
        return;
    ...
    d->staticWidget = QHTMLViewPrivate::SBNone;
}

void QHTMLView::addStaticObject(bool fixed)
{
    if (fixed) {
        d->fixedObjectsCount++;
    } else {
...
void QHTMLView::removeStaticObject(bool fixed)
{
    if (fixed) {
        d->fixedObjectsCount--;
    } else {
...
void QHTMLView::setVerticalScrollBarPolicy(Qt::ScrollBarPolicy policy)
{
#ifndef KHTML_NO_SCROLLBARS
    d->vpolicy = policy;
#ifdef QT_WIDGETS_LIB
...
void QHTMLView::setHorizontalScrollBarPolicy(Qt::ScrollBarPolicy policy)
{
#ifndef KHTML_NO_SCROLLBARS
    d->hpolicy = policy;
#ifdef QT_WIDGETS_LIB
...
void QHTMLView::clearCompletionHistory(const QString &name)
{
    //AFA
//    if (!d->formCompletions) {
//        d->formCompletions = new KConfig(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QLatin1Char('/') + "khtml/formcompletions");
...
void QHTMLView::addFormCompletionItem(const QString &name, const QString &value)
{
    if (!m_part->settings()->isFormCompletionEnabled()) {
        return;
    }
    ...
    //AFA d->formCompletions->group("").writeEntry(name, items);
}

void QHTMLView::addNonPasswordStorableSite(const QString &host)
{
    //AFA
//    if (!d->formCompletions) {
//        d->formCompletions = new KConfig(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QLatin1Char('/') + "khtml/formcompletions");
...
void QHTMLView::delNonPasswordStorableSite(const QString &host)
{
    //AFA
//    if (!d->formCompletions) {
//        d->formCompletions = new KConfig(QStandardPaths::writableLocation(QStandardPaths::GenericDataLocation) + QLatin1Char('/') + "khtml/formcompletions");
...
bool QHTMLView::nonPasswordStorableSite(const QString &host) const
{
    //AFA
    return true;
//    if (!d->formCompletions) {
...
// returns true if event should be swallowed
bool QHTMLView::dispatchMouseEvent(int eventId, DOM::NodeImpl *targetNode,
...
void QHTMLView::setIgnoreWheelEvents(bool e)
{
...
void QHTMLView::wheelEvent(QWheelEvent *e)
{
    ...
    if ((e->modifiers() & Qt::ControlModifier) == Qt::ControlModifier) {
        ...
        e->accept();
               ...
               ((e->orientation() == Qt::Vertical &&
                 ((d->ignoreWheelEvents && verticalScrollBar() && !verticalScrollBar()->isVisible())
                  || (e->delta() > 0 && contentsY() <= 0)
                  || (e->delta() < 0 && contentsY() >= contentsHeight() - visibleHeight())))
                ||
                (e->orientation() == Qt::Horizontal &&
                 ((d->ignoreWheelEvents && horizontalScrollBar() && !horizontalScrollBar()->isVisible())
...
#else
            QQuickScrollBar *hsb = horizontalScrollBar();
            QQuickScrollBar *vsb = verticalScrollBar();
            if ((h && ((d && hsb && hsb->position() == hsb->size()) || (!d && hsb && hsb->position() == hsb->size()))) ||
                    (!h && ((d && vsb && vsb->position() == vsb->size()) || (!d && vsb && vsb->position() == vsb->size())))) {
                e->accept();
                return;
            }
#endif
...
#ifndef QT_WIDGETS_LIB
void KHTMLView::hoverEnterEvent(QHoverEvent *event)
{

}

void KHTMLView::hoverMoveEvent(QHoverEvent *event)
{
    QMouseEvent mouseEvent(QEvent::MouseMove, event->pos(), Qt::NoButton, Qt::NoButton, Qt::NoModifier);
    mouseMoveEvent(&mouseEvent);
}

void KHTMLView::hoverLeaveEvent(QHoverEvent *event)
{

}
#endif

void QHTMLView::dragEnterEvent(QDragEnterEvent *ev)
{
    // Still overridden for BC reasons only...
    QScrollArea::dragEnterEvent(ev);
}

void QHTMLView::dropEvent(QDropEvent *ev)
{
    // Still overridden for BC reasons only...
    QScrollArea::dropEvent(ev);
}

void QHTMLView::focusInEvent(QFocusEvent *e)
{
...
#else
        static_cast<khtml::RenderWidget *>(fn->renderer())->widget()->setFocus(true);
#endif
    }
...
void QHTMLView::focusOutEvent(QFocusEvent *e)
{
    if (m_part) {
...
void QHTMLView::scrollContentsBy(int dx, int dy)
{
    if (!dx && !dy) {
        return;
    }
    ...
    if (d->shouldSmoothScroll && d->smoothScrollMode != SSMDisabled && m_part->xmlDocImpl() &&
        ...
        if (!doSmoothScroll && d->staticWidget == QHTMLViewPrivate::SBPartial && r.rects().size() <= 10) {
...
#else
    emit m_part->showToolTip(QPoint(-1, -1), QString());
#endif
    ...
    if (m_kwp->isRedirected()) {
        ...
        QHTMLView *v = m_kwp->rootViewPos(off);
        if (v) {
            w = v->widget();
        }
    ...
    if (m_kwp->isRedirected()) {
        ...
        if (viewport()) {
            off = viewport()->mapToItem(this, off).toPoint();
        }
    }
#endif
    ...
    if (d->staticWidget) {
        ...
        if (d->staticWidget == QHTMLViewPrivate::SBPartial
    ...
    if (m_kwp->isRedirected()) {
        const QRect rect(off.x(), off.y(), visibleWidth() * d->zoomLevel / 100, visibleHeight() * d->zoomLevel / 100);
        //AFA-FIXME w->scroll(dx, dy, rect);
        if (d->zoomLevel != 100) {
            update(rect); // without this update we are getting bad rendering when an iframe is zoomed in
        }
    }  else {
        //AFA-FIXME widget()->scroll(dx, dy, widget()->rect() & viewport()->rect());
    }
#endif
...
void QHTMLView::setupSmoothScrolling(int dx, int dy)
{
...
void QHTMLView::addChild(QWidget *child, int x, int y)
{
    if (!child) {
        return;
    }
...
//    qCDebug(KHTML_LOG) << "timer event " << e->timerId();
    if (e->timerId() == d->scrollTimerId) {
        ...
        case QHTMLViewPrivate::ScrollDown:
            ...
            break;
        case QHTMLViewPrivate::ScrollUp:
            if (contentsY() <= 0) {
                d->newScrollTimer(this, 0);
            } else {
                verticalScrollBar()->setValue(verticalScrollBar()->value() - d->scrollBy);
            ...
            break;
        case QHTMLViewPrivate::ScrollRight:
            ...
            break;
        case QHTMLViewPrivate::ScrollLeft:
            if (contentsX() <= 0) {
                d->newScrollTimer(this, 0);
            } else {
                horizontalScrollBar()->setValue(horizontalScrollBar()->value() - d->scrollBy);
    ...
    if (d->emitCompletedAfterRepaint) {
        bool full = d->emitCompletedAfterRepaint == QHTMLViewPrivate::CSFull;
        d->emitCompletedAfterRepaint = QHTMLViewPrivate::CSNone;
        if (full) {
...
void QHTMLView::checkExternalWidgetsPosition()
{
    QWidget *w;
    ...
    foreach (RenderWidget *r, toRemove)
        if ((w = d->visibleWidgets.take(r))) {
            ...
            w->move(0, -500000);
#else
            w->setPosition(QPoint(0, -500000));
#endif
        }
}

void QHTMLView::scheduleRelayout(khtml::RenderObject * /*clippedObj*/)
{
...
void QHTMLView::unscheduleRelayout()
{
    if (!d->layoutTimerId) {
        return;
    }
...
void QHTMLView::unscheduleRepaint()
{
    if (!d->repaintTimerId) {
        return;
    }
...
void QHTMLView::scheduleRepaint(int x, int y, int w, int h, bool asap)
{
...
void QHTMLView::complete(bool pendingAction)
{
//     qCDebug(KHTML_LOG) << "QHTMLView::complete()";
...
//         qCDebug(KHTML_LOG) << "requesting relayout now";
        ...
        d->emitCompletedAfterRepaint = pendingAction ?
                                       QHTMLViewPrivate::CSActionPending : QHTMLViewPrivate::CSFull;
    }
...
bool QHTMLView::caretKeyPressEvent(QKeyEvent *_ke)
{
