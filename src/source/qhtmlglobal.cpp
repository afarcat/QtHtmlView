...
QHTMLGlobal *QHTMLGlobal::s_self = nullptr;
unsigned long int QHTMLGlobal::s_refcnt = 0;
QString *QHTMLGlobal::s_about = nullptr;
QHTMLSettings *QHTMLGlobal::s_settings = nullptr;

static QLinkedList<QHTMLPart *> *s_parts = nullptr;
static QLinkedList<DOM::DocumentImpl *> *s_docs = nullptr;

QHTMLGlobal::QHTMLGlobal()
{
    assert(!s_self);
    s_self = this;
    ref();
...
QHTMLGlobal::~QHTMLGlobal()
{
    ...
    if (s_self == this) {
        ...
        delete s_about;
        delete s_settings;
        delete QHTMLSettings::avFamilies;
        if (s_parts) {
        ...
        s_parts = nullptr;
        s_docs = nullptr;
        QHTMLSettings::avFamilies = nullptr;
...
void QHTMLGlobal::ref()
{
    if (!s_refcnt && !s_self) {
        //qCDebug(KHTML_LOG) << "Creating QHTMLGlobal instance";
        // we can't use a staticdeleter here, because that would mean
        // that the QHTMLGlobal instance gets deleted from within a qPostRoutine, called
        ...
        // (Simon)
        new QHTMLGlobal; // does initial ref()
    } else {
        ++s_refcnt;
    }
...
void QHTMLGlobal::registerPart(QHTMLPart *part)
{
    //qCDebug(KHTML_LOG) << part;
    if (!s_parts) {
        s_parts = new QLinkedList<QHTMLPart *>;
    }
...
void QHTMLGlobal::deregisterPart(QHTMLPart *part)
{
    //qCDebug(KHTML_LOG) << part;
    assert(s_parts);
...
void QHTMLGlobal::registerDocumentImpl(DOM::DocumentImpl *doc)
{
...
void QHTMLGlobal::deregisterDocumentImpl(DOM::DocumentImpl *doc)
{
    //qCDebug(KHTML_LOG) << doc;
    assert(s_docs);
...
const QString &QHTMLGlobal::aboutData()
{
    assert(s_self);
...
QHTMLSettings *QHTMLGlobal::defaultHTMLSettings()
{
    assert(s_self);
    if (!s_settings) {
        s_settings = new QHTMLSettings();
    }

    return s_settings;
}

void QHTMLGlobal::finalCheck()
{
#ifndef NDEBUG
    if (s_refcnt) {
        if (s_parts && !s_parts->isEmpty()) {
            Q_FOREACH (QHTMLPart *part, *s_parts) {
