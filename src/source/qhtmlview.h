 ...
 **/
class KHTML_EXPORT QHTMLView : public QScrollArea, public khtml::KHTMLWidget
{
    Q_OBJECT
...
public:
    ...
    /**
     * Returns a pointer to the QHTMLPart that is
     * rendering the page.
     **/
    QHTMLPart *part() const
    {
        return m_part;
    }
...
Q_SIGNALS:
     ...
     * Use @ref QHTMLPart completed() signal instead.
     */
    void finishedLayout();
    void cleared();
    void zoomView(int);
...
private:
    ...
    bool focusNodeWithAccessKey(QChar c, QHTMLView *caller = nullptr);
    QMap< DOM::ElementImpl *, QChar > buildFallbackAccessKeys() const;
    void displayAccessKeys(QHTMLView *caller, QHTMLView *origview, QVector< QChar > &taken, bool use_fallbacks);
     ...
     * @return a QHTMLPart::CaretDisplay value
     */
     ...
     *      defined by QHTMLPart::CaretDisplayPolicy
     */
