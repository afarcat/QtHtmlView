...
//AFA #include "ui_htmlpageinfo.h"

//AFA #include "khtmlviewbar.h"
//AFA #include "khtml_pagecache.h"
...
//AFA #include <kparts/browseropenorsavequestion.h>
//AFA #include <kparts/guiactivateevent.h>

//AFA #include <kacceleratormanager.h>
...
//AFA #include <kstringhandler.h>
#include <kencodingdetector.h>
//#include <kencodingprober.h>
#include <kio/job.h>
//AFA #include <kio/jobuidelegate.h>
//AFA #include <kio/global.h>
//AFA #include <kio/pixmaploader.h>
//AFA #include <kio/hostinfo.h>
//AFA #include <kprotocolmanager.h>
//AFA #include "khtml_debug.h"
//AFA #include <kjobwidgets.h>
//AFA #include <kmessagebox.h>
//AFA #include <kstandardaction.h>
//AFA #include <kstandardguiitem.h>
//AFA #include <kactioncollection.h>
//AFA #include <kmimetypetrader.h>
#include <qtemporaryfile.h>
//AFA #include <ktoolinvocation.h>
//AFA #include <kurlauthorized.h>
//AFA #include <kparts/browserinterface.h>
//AFA #include <kparts/scriptableextension.h>
//AFA #include <kparts/liveconnectextension.h>
//AFA #include <kactionmenu.h>
//AFA #include <ktoggleaction.h>
//AFA #include <kcodecaction.h>
//AFA #include <kselectaction.h>

//AFA #include <QDBusConnection>
//AFA #include <ksslinfodialog.h>
//AFA #include <ksslsettings.h>

//AFA #include <QDBusInterface>
#include <QMimeData>
//AFA #include <kfileitem.h>
//AFA #include <kurifilter.h>
//AFA #include <kurllabel.h>
//AFA #include <kurlmimedata.h>
...
//AFA #include "kpassivepopup.h"
//AFA #include "rendering/render_form.h"
...
bool QHTMLPartPrivate::s_dnsInitialised = false;
...
{
public:
    PartStyleSheetLoader(QHTMLPart *part, DOM::DOMString url, DocLoader *dl)
    {
        m_part = part;
...
void QHTMLPart::init(QHTMLView *view)
{
    ...
    if (!parentPart()) {
        ...
        //AFA d->m_topViewBar = new QHTMLViewBar(QHTMLViewBar::Top, d->m_view, widget);
        //AFA d->m_bottomViewBar = new QHTMLViewBar(QHTMLViewBar::Bottom, d->m_view, widget);

        //AFA layout->addWidget(d->m_topViewBar);
    ...
    //AFA d->m_guiProfile = prof;
    d->m_extension = new KParts::BrowserExtension(this);
    d->m_extension->setObjectName("KHTMLBrowserExtension");
    //AFA d->m_hostExtension = new KParts::BrowserHostExtension(this);
    //AFA d->m_statusBarExtension = new KParts::StatusBarExtension(this);
#if ENABLE(KJS)
    d->m_scriptableExtension = new KJS::KHTMLPartScriptable(this);
#endif
    //AFA new KHTMLTextExtension(this);
    //AFA new KHTMLHtmlExtension(this);
#if ENABLE(KJS)
...
//         qCWarning(KHTML_LOG) << "0000000end " << d->m_autoDetectLanguage << " " << QTextCodec::codecForLocale()->mibEnum();
    ...
    QHTMLSettings::KSmoothScrollingMode ssm = d->m_settings->smoothScrolling();
    if (ssm == QHTMLSettings::KSmoothScrollingDisabled) {
        d->m_view->setSmoothScrollingModeDefault(QHTMLView::SSMDisabled);
    } else if (ssm == QHTMLSettings::KSmoothScrollingWhenEfficient) {
        d->m_view->setSmoothScrollingModeDefault(QHTMLView::SSMWhenEfficient);
    } else {
        d->m_view->setSmoothScrollingModeDefault(QHTMLView::SSMEnabled);
    }

    if (d->m_bDNSPrefetchIsDefault && !onlyLocalReferences()) {
        QHTMLSettings::KDNSPrefetch dpm = d->m_settings->dnsPrefetch();
        if (dpm == QHTMLSettings::KDNSPrefetchDisabled) {
            d->m_bDNSPrefetch = DNSPrefetchDisabled;
        } else if (dpm == QHTMLSettings::KDNSPrefetchOnlyWWWAndSLD) {
    ...
    if (!QHTMLPartPrivate::s_dnsInitialised && d->m_bDNSPrefetch != DNSPrefetchDisabled) {
        //AFA KIO::HostInfo::setCacheSize(sDNSCacheSize);
        //AFA KIO::HostInfo::setTTL(sDNSTTLSeconds);
        QHTMLPartPrivate::s_dnsInitialised = true;
    }
...
//        new QHTMLPartIface(this); // our "adaptor"
//        for (int i = 1;; ++i)
//            if (QDBusConnection::sessionBus().registerObject(QString("/KHTML/%1/widget").arg(i), this)) {
//                break;
//            } else if (i == 0xffff) {
//                qFatal("Something is very wrong in QHTMLPart!");
//            }
//    }
...
bool QHTMLPart::restoreURL(const QUrl &url)
{
    ...
    */
    ...
    d->m_bJScriptEnabled = QHTMLGlobal::defaultHTMLSettings()->isJavaScriptEnabled(url.host());
    setDebugScript(QHTMLGlobal::defaultHTMLSettings()->isJavaScriptDebugEnabled());
    d->m_bJavaEnabled = QHTMLGlobal::defaultHTMLSettings()->isJavaEnabled(url.host());
    d->m_bPluginsEnabled = QHTMLGlobal::defaultHTMLSettings()->isPluginsEnabled(url.host());
    ...
    //AFA KHTMLPageCache::self()->fetchData(d->m_cacheId, this, SLOT(slotRestoreData(QByteArray)));
...
bool QHTMLPartPrivate::isLocalAnchorJump(const QUrl &url)
{
...
void QHTMLPartPrivate::executeAnchorJump(const QUrl &url, bool lockHistory)
{
...
bool QHTMLPart::openUrl(const QUrl &url)
{
    // qCDebug(KHTML_LOG) << this << "opening" << url;

    //AFA-modify
    if (!url.isValid() || url.scheme().isEmpty()) {
        qCWarning(KHTML_LOG) << "Invalid URL:" << url;
    ...
    if (isFrameSet && d->isLocalAnchorJump(url) && browserArgs.softReload) {
        ...
        for (; it != end; ++it) {
            QHTMLPart *const part = qobject_cast<QHTMLPart *>((*it)->m_part.data());
            if (part) {
    ...
    if (d->m_restored) {
        ...
        //AFA d->m_cachePolicy = KProtocolManager::cacheControl();
    }
...
bool QHTMLPart::closeUrl()
{
    if (d->m_job) {
        //AFA KHTMLPageCache::self()->cancelEntry(d->m_cacheId);
        d->m_job->kill();
        d->m_job = nullptr;
    }
    ...
    //AFA d->m_cachePolicy = KProtocolManager::cacheControl(); // reset cache policy

    disconnect(d->m_view, SIGNAL(finishedLayout()), this, SLOT(restoreScrollPosition()));

    //AFA KHTMLPageCache::self()->cancelFetch(this);
...
void QHTMLPart::setArguments(const KParts::OpenUrlArguments &arguments)
{
    d->m_arguments = arguments;
}

KParts::OpenUrlArguments QHTMLPart::arguments() const
{
    return d->m_arguments;
}

DOM::HTMLDocumentImpl *QHTMLPart::htmlDocument() const
{
    if (d->m_doc && d->m_doc->isHTMLDocument()) {
...
DOM::DocumentImpl *QHTMLPart::document() const
{
    return d->m_doc;
}

QString QHTMLPart::documentSource() const
{
    QString sourceStr;
...
#if ENABLE(GUI)
QHTMLViewBar *QHTMLPart::pTopViewBar() const
{
    if (const_cast<QHTMLPart *>(this)->parentPart()) {
        return const_cast<QHTMLPart *>(this)->parentPart()->pTopViewBar();
    }
    return d->m_topViewBar;
}

QHTMLViewBar *QHTMLPart::pBottomViewBar() const
{
    if (const_cast<QHTMLPart *>(this)->parentPart()) {
        return const_cast<QHTMLPart *>(this)->parentPart()->pBottomViewBar();
    }
    return d->m_bottomViewBar;
}
#endif

void QHTMLPart::setStatusMessagesEnabled(bool enable)
{
    d->m_statusMessagesEnabled = enable;
}

KJS::Interpreter *QHTMLPart::jScriptInterpreter()
{
#if ENABLE(KJS)
...
bool QHTMLPart::statusMessagesEnabled() const
{
    return d->m_statusMessagesEnabled;
}

void QHTMLPart::setJScriptEnabled(bool enable)
{
#if ENABLE(KJS)
...
bool QHTMLPart::jScriptEnabled() const
{
...
void QHTMLPart::setDNSPrefetch(DNSPrefetch pmode)
{
    d->m_bDNSPrefetch = pmode;
    d->m_bDNSPrefetchIsDefault = false;
}

QHTMLPart::DNSPrefetch QHTMLPart::dnsPrefetch() const
{
    if (onlyLocalReferences()) {
...
void QHTMLPart::setMetaRefreshEnabled(bool enable)
{
    d->m_metaRefreshEnabled = enable;
}

bool QHTMLPart::metaRefreshEnabled() const
{
    return d->m_metaRefreshEnabled;
}

KJSProxy *QHTMLPart::jScript()
{
    if (!jScriptEnabled()) {
        return nullptr;
    }
...
QVariant QHTMLPart::crossFrameExecuteScript(const QString &target,  const QString &script)
{
    QHTMLPart *destpart = this;
...
KJSErrorDlg *QHTMLPart::jsErrorExtension()
{
...
void QHTMLPart::removeJSErrorExtension()
{
    if (parentPart()) {
...
void QHTMLPart::disableJSErrorExtension()
{
    removeJSErrorExtension();
...
void QHTMLPart::launchJSConfigDialog()
{
    QStringList args;
    args << "khtml_java_js";
    //AFA KToolInvocation::kdeinitExec("kcmshell5", args);
}

QVariant QHTMLPart::executeScript(const QString &filename, int baseLine, const DOM::Node &n, const QString &script)
{
#ifdef KJS_VERBOSE
...
QVariant QHTMLPart::executeScript(const QString &script)
{
    return executeScript(DOM::Node(), script);
}

QVariant QHTMLPart::executeScript(const DOM::Node &n, const QString &script)
{
#ifdef KJS_VERBOSE
    ...
    if (comp.complType() == KJS::Throw && comp.value()) {
        ...
        if (dlg) {
            QString msg = KJS::exceptionToString(
                              proxy->interpreter()->globalExec(), comp.value());
            //AFA dlg->addError(i18n("<qt><b>Error</b>: node %1: %2</qt>",
            //AFA                    n.nodeName().string(), Qt::escape(msg)));
        }
    }
...
void QHTMLPart::setJavaEnabled(bool enable)
{
    d->m_bJavaForce = enable;
    d->m_bJavaOverride = true;
}

bool QHTMLPart::javaEnabled() const
{
    if (onlyLocalReferences()) {
        return false;
    }
...
void QHTMLPart::setPluginsEnabled(bool enable)
{
    d->m_bPluginsForce = enable;
    d->m_bPluginsOverride = true;
}

bool QHTMLPart::pluginsEnabled() const
{
    if (onlyLocalReferences()) {
        return false;
    }
...
void QHTMLPart::slotDebugDOMTree()
{
    if (d->m_doc) {
    ...
    for (; it != end; ++it)
        if (!(*it)->m_part.isNull() && (*it)->m_part.data()->inherits("KHTMLPart")) {
            ...
            static_cast<QHTMLPart *>(p)->slotDebugDOMTree();
        }
    s_DOMTreeIndentLevel = indentLevel;
}

void QHTMLPart::slotDebugScript()
{
#if ENABLE(KJS)
    if (jScript()) {
...
void QHTMLPart::slotDebugRenderTree()
{
#ifndef NDEBUG
    if (d->m_doc) {
...
void QHTMLPart::slotDebugFrameTree()
{
    khtml::ChildFrame::dumpFrameTree(this);
}

void QHTMLPart::slotStopAnimations()
{
    stopAnimations();
}

void QHTMLPart::setAutoloadImages(bool enable)
{
...
#if ENABLE(GUI)
    //AFA unplugActionList("loadImages");
    ...
    if (d->m_paLoadImages) {
        ...
        //AFA plugActionList("loadImages", lst);
    }
#endif
}

bool QHTMLPart::autoloadImages() const
{
    if (d->m_doc) {
...
bool QHTMLPart::openFile()
{
    return true;
}

DOM::HTMLDocumentImpl *QHTMLPart::docImpl() const
{
...
DOM::DocumentImpl *QHTMLPart::xmlDocImpl() const
{
    if (d) {
        return d->m_doc;
    }
...
void QHTMLPart::slotInfoMessage(KJob *kio_job, const QString &msg)
{
...
void QHTMLPart::setPageSecurity(PageSecurity sec)
{
    emit d->m_extension->setPageSecurity(sec);
}

void QHTMLPart::slotData(KIO::Job *kio_job, const QByteArray &data)
{
    assert(d->m_job == kio_job);
    Q_ASSERT(kio_job);
    Q_UNUSED(kio_job);
    ...
    if (!d->m_workingURL.isEmpty()) {
        ...
        if (d->m_cachePolicy == KIO::CC_Refresh/*AFA && !d->m_extension->browserArguments().softReload*/) {
        ...
        //AFA d->m_cacheId = KHTMLPageCache::self()->createCacheEntry();
    ...
    //AFA KHTMLPageCache::self()->addData(d->m_cacheId, data);
    write(data.data(), data.size());
}

void QHTMLPart::slotRestoreData(const QByteArray &data)
{
    // The first data ?
    if (!d->m_workingURL.isEmpty()) {
...
void QHTMLPart::showError(KJob *job)
{
    ...
    if ((d->m_doc && d->m_doc->parsing()) || d->m_workingURL.isEmpty()) { // if we got any data already
        //AFA job->uiDelegate()->showErrorMessage();
    } else {
...
void QHTMLPart::htmlError(int errorCode, const QString &text, const QUrl &reqUrl)
{
...
void QHTMLPart::slotFinished(KJob *job)
{
    d->m_job = nullptr;
    d->m_jobspeed = 0L;
    ...
    KIO::Job *tjob = ::qobject_cast<KIO::Job *>(job);
    ...
    //AFA KHTMLPageCache::self()->endData(d->m_cacheId);
...
MimeType QHTMLPartPrivate::classifyMimeType(const QString &mimeStr)
{
...
void QHTMLPart::begin(const QUrl &url, int xOffset, int yOffset)
{
...
#if ENABLE(KJS)
        d->m_openableSuppressedPopups = 0;
        foreach (QHTMLPart *part, d->m_suppressedPopupOriginParts) {
            if (part) {
    ...
    if (url.isValid()) {
        ...
        QHTMLGlobal::vLinks()->insert(urlString);
        QString urlString2 = url.toDisplayString();
        if (urlString != urlString2) {
            QHTMLGlobal::vLinks()->insert(urlString2);
        }
    }
    ...
    d->m_doc->docLoader()->setShowAnimations(QHTMLGlobal::defaultHTMLSettings()->showAnimations());
    emit docCreated();
    ...
    setAutoloadImages(QHTMLGlobal::defaultHTMLSettings()->autoLoadImages());
    QString userStyleSheet = QHTMLGlobal::defaultHTMLSettings()->userStyleSheet();
...
void QHTMLPart::write(const char *data, int len)
{
    if (!d->m_decoder) {
...
// ### KDE5: remove
void QHTMLPart::setAlwaysHonourDoctype(bool b)
{
    d->m_bStrictModeQuirk = !b;
}

void QHTMLPart::write(const QString &str)
{
    if (str.isNull()) {
        return;
    }
...
bool QHTMLPart::doOpenStream(const QString &mimeType)
{
    QMimeDatabase db;
...
bool QHTMLPart::doWriteStream(const QByteArray &data)
{
    write(data.data(), data.size());
    return true;
}

bool QHTMLPart::doCloseStream()
{
    end();
    return true;
}

void QHTMLPart::paint(QPainter *p, const QRect &rc, int yOff, bool *more)
{
    if (!d->m_view) {
        return;
    }
...
void QHTMLPart::stopAnimations()
{
    if (d->m_doc) {
        d->m_doc->docLoader()->setShowAnimations(QHTMLSettings::KAnimationDisabled);
    }

    ConstFrameIt it = d->m_frames.constBegin();
    const ConstFrameIt end = d->m_frames.constEnd();
    for (; it != end; ++it) {
        if (QHTMLPart *p = qobject_cast<QHTMLPart *>((*it)->m_part.data())) {
            p->stopAnimations();
        }
    }
}

void QHTMLPart::resetFromScript()
{
    closeUrl();
    d->m_bComplete = false;
    d->m_bLoadEventEmitted = false;
...
void QHTMLPart::slotFinishedParsing()
{
    d->m_doc->setParsing(false);
...
void QHTMLPart::slotLoaderRequestStarted(khtml::DocLoader *dl, khtml::CachedObject *obj)
{
    if (obj && obj->type() == khtml::CachedObject::Image && d->m_doc && d->m_doc->docLoader() == dl) {
        QHTMLPart *p = this;
        while (p) {
            QHTMLPart *const op = p;
...
static bool isAncestorOrSamePart(QHTMLPart *p1, QHTMLPart *p2)
{
    QHTMLPart *p = p2;
    do {
        if (p == p1) {
...
void QHTMLPart::slotLoaderRequestDone(khtml::DocLoader *dl, khtml::CachedObject *obj)
{
    if (obj && obj->type() == khtml::CachedObject::Image && d->m_doc && d->m_doc->docLoader() == dl) {
        QHTMLPart *p = this;
        while (p) {
...
void QHTMLPart::slotProgressUpdate()
{
    int percent;
    ...
    if (d->m_statusMessagesEnabled) {
        if (d->m_bComplete) {
            ...
            emit d->m_extension->infoMessage(QString("%1 Images of %2 loaded.").arg(d->m_loadedObjects).arg(d->m_totalObjectCount));
        }
    }
...
void QHTMLPart::slotJobSpeed(KJob * /*job*/, unsigned long speed)
{
    d->m_jobspeed = speed;
    if (!parentPart()) {
...
void QHTMLPart::slotJobPercent(KJob * /*job*/, unsigned long percent)
{
...
void QHTMLPart::slotJobDone(KJob * /*job*/)
{
    d->m_jobPercent = 100;
...
void QHTMLPart::slotUserSheetStatDone(KJob *_job)
{
    using namespace KIO;
...
bool QHTMLPartPrivate::isFullyLoaded(bool *pendingRedirections) const
{
...
//   qCDebug(KHTML_LOG) << "   complete: " << d->m_bComplete;
    ...
    //AFA d->m_paUseStylesheet->setItems(sheets);

    //AFA d->m_paUseStylesheet->setEnabled(sheets.count() > 2);
    if (sheets.count() > 2) {
        //AFA d->m_paUseStylesheet->setCurrentItem(qMax(sheets.indexOf(d->m_sheetUsed), 0));
        slotUseStylesheet();
    }
...
void QHTMLPart::checkEmitLoadEvent()
{
    bool fullyLoaded, pendingChildRedirections;
    fullyLoaded = d->isFullyLoaded(&pendingChildRedirections);
...
QUrl QHTMLPart::completeURL(const QString &url)
{
    if (!d->m_doc) {
        return QUrl(url);
    }
...
QString QHTMLPartPrivate::codeForJavaScriptURL(const QString &u)
{
    return QUrl::fromPercentEncoding(u.right(u.length() - 11).toUtf8());
}

void QHTMLPartPrivate::executeJavascriptURL(const QString &u)
{
...
bool QHTMLPartPrivate::isJavaScriptURL(const QString &url)
{
...
void QHTMLPart::scheduleRedirection(int delay, const QString &url, bool doLockHistory)
{
...
void QHTMLPartPrivate::clearRedirection()
{
    m_delayRedirect = 0;
    m_redirectURL.clear();
...
//        qCWarning(KHTML_LOG) << "QHTMLPart::scheduleRedirection: Redirection from " << cUrl << " to " << url << " REJECTED!";
//        emit completed();
//        return;
//    }
...
void QHTMLPart::slotRedirection(KIO::Job *, const QUrl &url)
{
...
bool QHTMLPart::setEncoding(const QString &name, bool override)
{
    d->m_encoding = name;
...
QString QHTMLPart::encoding() const
{
...
QString QHTMLPart::defaultEncoding() const
{
...
void QHTMLPart::setUserStyleSheet(const QUrl &url)
{
    if (d->m_doc && d->m_doc->docLoader()) {
...
void QHTMLPart::setUserStyleSheet(const QString &styleSheet)
{
    if (d->m_doc) {
...
bool QHTMLPart::gotoAnchor(const QString &name)
{
    if (!d->m_doc) {
        return false;
    }
...
void QHTMLPart::setStandardFont(const QString &name)
{
    d->m_settings->setStdFontName(name);
}

void QHTMLPart::setFixedFont(const QString &name)
{
    d->m_settings->setFixedFontName(name);
}

void QHTMLPart::setURLCursor(const QCursor &c)
{
    d->m_linkCursor = c;
}

QCursor QHTMLPart::urlCursor() const
{
    return d->m_linkCursor;
}

bool QHTMLPart::onlyLocalReferences() const
{
    return d->m_onlyLocalReferences;
}

void QHTMLPart::setOnlyLocalReferences(bool enable)
{
    d->m_onlyLocalReferences = enable;
}

bool QHTMLPart::forcePermitLocalImages() const
{
    return d->m_forcePermitLocalImages;
}

void QHTMLPart::setForcePermitLocalImages(bool enable)
{
    d->m_forcePermitLocalImages = enable;
}

void QHTMLPartPrivate::setFlagRecursively(
    bool QHTMLPartPrivate::*flag, bool value)
{
...
static void setCaretInvisibleIfNeeded(QHTMLPart *part)
{
    // On contenteditable nodes, don't hide the caret
    if (!khtml::QHTMLPartAccessor::caret(part).caretPos().node()->isContentEditable()) {
        part->setCaretVisible(false);
    }
}

void QHTMLPart::setCaretMode(bool enable)
{
    // qCDebug(KHTML_LOG) << enable;
    if (isCaretMode() == enable) {
        return;
    }
    d->setFlagRecursively(&QHTMLPartPrivate::m_caretMode, enable);
...
bool QHTMLPart::isCaretMode() const
{
    return d->m_caretMode;
}

void QHTMLPart::setEditable(bool enable)
{
    if (isEditable() == enable) {
        return;
    }
    d->setFlagRecursively(&QHTMLPartPrivate::m_designMode, enable);
    // FIXME: this won't work on frames as expected
    if (!isCaretMode()) {
        if (enable) {
            initCaret();
...
bool QHTMLPart::isEditable() const
{
    return d->m_designMode;
}

khtml::EditorContext *QHTMLPart::editorContext() const
{
    return &d->editor_context;
}

void QHTMLPart::setCaretPosition(DOM::Node node, long offset, bool extendSelection)
{
    Q_UNUSED(node);
    Q_UNUSED(offset);
...
QHTMLPart::CaretDisplayPolicy QHTMLPart::caretDisplayPolicyNonFocused() const
{
#if 0
#ifndef KHTML_NO_CARET
...
void QHTMLPart::setCaretDisplayPolicyNonFocused(CaretDisplayPolicy policy)
{
    Q_UNUSED(policy);
#if 0
#ifndef KHTML_NO_CARET
...
void QHTMLPart::setCaretVisible(bool show)
{
    if (show) {
...
bool QHTMLPart::initFindNode(bool selection, bool reverse, bool fromCursor)
{
#if ENABLE(FIND)
...
void QHTMLPart::slotFind()
{
    ...
    static_cast<QHTMLPart *>(part)->findText();
}

void QHTMLPart::slotFindNext()
{
    KParts::ReadOnlyPart *part = currentFrame();
    if (!part) {
        return;
    ...
    static_cast<QHTMLPart *>(part)->findTextNext();
}

void QHTMLPart::slotFindPrev()
{
    KParts::ReadOnlyPart *part = currentFrame();
    if (!part) {
        return;
    ...
    static_cast<QHTMLPart *>(part)->findTextNext(true);     // reverse
}

void QHTMLPart::slotFindDone()
{
    // ### remove me
}

void QHTMLPart::slotFindAheadText()
{
#if ENABLE(FIND)
    QHTMLPart *part = qobject_cast<QHTMLPart *>(currentFrame());
    if (!part) {
        return;
    }
    part->findText();
...
void QHTMLPart::slotFindAheadLink()
{
#if ENABLE(FIND)
    QHTMLPart *part = qobject_cast<QHTMLPart *>(currentFrame());
    if (!part) {
...
void QHTMLPart::enableFindAheadActions(bool)
{
    // ### remove me
}

void QHTMLPart::slotFindDialogDestroyed()
{
    // ### remove me
}

void QHTMLPart::findText()
{
    if (parentPart()) {
...
void QHTMLPart::findText(const QString &str, long options, QWidget *parent/*AFA, KFindDialog *findDialog*/)
{
#if ENABLE(FIND)
    if (parentPart()) {
...
// New method
bool QHTMLPart::findTextNext(bool reverse)
{
#if ENABLE(FIND)
    if (parentPart()) {
...
bool QHTMLPart::pFindTextNextInThisFrame(bool reverse)
{
#if ENABLE(FIND)
    return d->m_find.findTextNext(reverse);
#else
...
QString QHTMLPart::selectedTextAsHTML() const
{
...
QString QHTMLPart::selectedText() const
{
    bool hasNewLine = true;
    bool seenTDTag = false;
    QString text;
    const Selection &sel = d->editor_context.m_selection;
    DOM::NodeImpl *n = sel.start().node();
    while (n != nullptr) {
        if (n->nodeType() == DOM::Node::TEXT_NODE && n->renderer()) {
            DOM::DOMStringImpl *dstr = static_cast<DOM::TextImpl *>(n)->renderString();
            ...
            unsigned short id = n->id();
            switch (id) {
            case ID_TEXTAREA:
                text += static_cast<HTMLTextAreaElementImpl *>(n)->value().string();
                break;
            case ID_INPUT:
                if (static_cast<HTMLInputElementImpl *>(n)->inputType() != HTMLInputElementImpl::PASSWORD) {
                    text += static_cast<HTMLInputElementImpl *>(n)->value().string();
                }
                break;
            case ID_SELECT:
                text += static_cast<HTMLSelectElementImpl *>(n)->value().string();
                break;
            case ID_BR:
                text += "\n";
                hasNewLine = true;
                break;
            case ID_IMG:
                text += static_cast<HTMLImageElementImpl *>(n)->altText().string();
                break;
            case ID_TD:
                break;
            case ID_TH:
        ...
        while (next == nullptr && n->parentNode() != nullptr) {
            n = n->parentNode();
            next = n->nextSibling();
            unsigned short id = n->id();
            switch (id) {
...
QString QHTMLPart::simplifiedSelectedText() const
{
...
bool QHTMLPart::hasSelection() const
{
    return !d->editor_context.m_selection.isEmpty() && !d->editor_context.m_selection.isCollapsed();
}

#if ENABLE(DOM)
DOM::Range QHTMLPart::selection() const
{
    return d->editor_context.m_selection.toRange();
}
#endif

void QHTMLPart::selection(DOM::Node &s, long &so, DOM::Node &e, long &eo) const
{
#if ENABLE(DOM)
...
#else
    NodeImpl *start, *end;
    d->editor_context.m_selection.getRange(start, so, end, eo);
    s = start;
    e = end;
#endif
}
...
#if ENABLE(DOM)
void QHTMLPart::setSelection(const DOM::Range &r)
{
    setCaret(r);
}
#endif

const Selection &QHTMLPart::caret() const
{
    return d->editor_context.m_selection;
}

const Selection &QHTMLPart::dragCaret() const
{
    return d->editor_context.m_dragCaret;
}

void QHTMLPart::setCaret(const Selection &s, bool closeTyping)
{
...
void QHTMLPart::setDragCaret(const DOM::Selection &dragCaret)
{
...
void QHTMLPart::invalidateSelection()
{
    clearCaretRectIfNeeded();
    d->editor_context.m_selection.setNeedsLayout();
...
void QHTMLPart::setSelectionVisible(bool flag)
{
...
#if 1
void QHTMLPart::slotClearSelection()
{
    if (!isCaretMode()
...
void QHTMLPart::clearCaretRectIfNeeded()
{
...
void QHTMLPart::setFocusNodeIfNeeded(const Selection &s)
{
...
void QHTMLPart::selectionLayoutChanged()
{
...
void QHTMLPart::notifySelectionChanged(bool closeTyping)
{
...
void QHTMLPart::timerEvent(QTimerEvent *e)
{
    if (e->timerId() == d->editor_context.m_caretBlinkTimer) {
        ...
        //AFA KIO::HostInfo::prefetchHost(d->m_DNSPrefetchQueue.dequeue());
...
bool QHTMLPart::mayPrefetchHostname(const QString &name)
{
...
void QHTMLPart::paintCaret(QPainter *p, const QRect &rect) const
{
    if (d->editor_context.m_caretPaint) {
...
void QHTMLPart::paintDragCaret(QPainter *p, const QRect &rect) const
{
    d->editor_context.m_dragCaret.paintCaret(p, rect);
}

DOM::Editor *QHTMLPart::editor() const
{
    if (!d->editor_context.m_editor) {
        const_cast<QHTMLPart *>(this)->d->editor_context.m_editor = new DOM::Editor(const_cast<QHTMLPart *>(this));
    }
    return d->editor_context.m_editor;
}

void QHTMLPart::resetHoverText()
{
    if (!d->m_overURL.isEmpty()) { // Only if we were showing a link
        ...
        //AFA emit d->m_extension->mouseOverInfo(KFileItem());
    }
}

void QHTMLPart::overURL(const QString &url, const QString &target, bool /*shiftPressed*/)
{
    ...
    const QString com = ""; //AFA item.mimeComment();
    ...
    if (u.isLocalFile()) {
        ...
        if (info.isSymLink()) {
            ...
            if (info.size() < 1024) {
                //AFA text = i18np("%2 (%1 byte)", "%2 (%1 bytes)", (long) info.size(), text2);    // always put the URL last, in case it contains '%'
                text = i18n("%2 (%1 bytes)").arg(info.size()).arg(text2);
            } else {
                float d = (float) info.size() / 1024.0;
                //AFA text = i18n("%2 (%1 K)", QLocale().toString(d, 'f', 2), text2.toUtf8()); // was %.2f
                text = i18n("%2 (%1 K)").arg(QString::number(d, 'f', 2)).arg(text2);
            }
            text += "  ";
            text += com;
...
// that active part.
//
bool QHTMLPart::urlSelected(const QString &url, int button, int state, const QString &_target, const KParts::OpenUrlArguments &_args, const KParts::BrowserArguments &_browserArgs)
{
    ...
    if (button == Qt::LeftButton && (state & Qt::ShiftModifier)) {
        //AFA KIO::MetaData metaData;
        //AFA metaData.insert("referrer", d->m_referrer);
        //AFA KHTMLPopupGUIClient::saveURL(d->m_view, i18n("Save As"), cURL, metaData);
        return false;
    }
...
void QHTMLPart::slotViewDocumentSource()
{
...
void QHTMLPart::slotViewPageInfo()
{
//    Ui_KHTMLInfoDlg ui;
...
void QHTMLPart::slotViewFrameSource()
{
...
//        long cacheId = static_cast<QHTMLPart *>(frame)->d->m_cacheId;
...
QUrl QHTMLPart::backgroundURL() const
{
...
void QHTMLPart::slotSaveBackground()
{
...
void QHTMLPart::slotSaveDocument()
{
    QUrl srcURL(url());
...
void QHTMLPart::slotSetEncoding(const QString &enc)
{
    //AFA d->m_autoDetectLanguage = KEncodingProber::None;
    setEncoding(enc, true);
}

//void QHTMLPart::slotAutomaticDetectionLanguage(KEncodingProber::ProberType scri)
//{
//    d->m_autoDetectLanguage = scri;
//    setEncoding(QString(), false);
//}

void QHTMLPart::slotUseStylesheet()
{
//    if (d->m_doc) {
...
#if ENABLE(KJS)
KParts::ScriptableExtension *QHTMLPart::scriptableExtension(const DOM::NodeImpl *frame)
{
    const ConstFrameIt end = d->m_objects.constEnd();
...
void QHTMLPart::loadFrameElement(DOM::HTMLPartContainerElementImpl *frame, const QString &url,
                                 const QString &frameName, const QStringList &params, bool isIFrame)
{
    ...
    if (!child->m_part) {
        ...
        // (async ones are discovered by the presence of the QHTMLRun)
    ...
    // Since we don't specify args here a QHTMLRun will be used to determine the
    ...
    // use that inside QHTMLPart::openUrl to route things appropriately.
    child->m_bCompleted = false;
    if (!requestObject(child, u)
#if ENABLE(RUN)
            && !child->m_run
#endif
            ) {
...
QString QHTMLPart::requestFrameName()
{
    return QString::fromLatin1("<!--frame %1-->").arg(d->m_frameNameId++);
}

bool QHTMLPart::loadObjectElement(DOM::HTMLPartContainerElementImpl *frame, const QString &url,
...
bool QHTMLPart::requestObject(khtml::ChildFrame *child, const QUrl &url, const KParts::OpenUrlArguments &_args,
                              const KParts::BrowserArguments &browserArgs)
{
    ...
    // no need to QHTMLRun to figure out the mimetype"
    ...
    if (args.mimeType().isEmpty()) {
        // qCDebug(KHTML_LOG) << "Running new QHTMLRun for" << this << "and child=" << child;
#if ENABLE(RUN)
        child->m_run = new QHTMLRun(this, child, url, child->m_args, child->m_browserArgs, true);
#endif
...
void QHTMLPart::childLoadFailure(khtml::ChildFrame *child)
{
    child->m_bCompleted = true;
...
bool QHTMLPart::processObjectRequest(khtml::ChildFrame *child, const QUrl &_url, const QString &mimetype)
{
...
//            int disposition = 0;
//            if (QHTMLRun *run = child->m_run.data()) {
...
//        // Now, for frames and iframes, we always create a QHTMLPart anyway,
//        // doing it in advance when registering the frame. So we want the
//        // actual creation only for objects here.
//        if (child->m_type == khtml::ChildFrame::Object) {
//            QMimeType mime = db.mimeTypeForName(mimetype);
//            if (mime.isValid()) {
//                // Even for objects, however, we want to force a QHTMLPart for
...
//    }
    ...
    // but it's useless if we had to use a QHTMLRun instance, as the
...
bool QHTMLPart::navigateLocalProtocol(khtml::ChildFrame * /*child*/, KParts::ReadOnlyPart *inPart,
                                      const QUrl &url)
{
    if (!qobject_cast<QHTMLPart *>(inPart)) {
        return false;
    }

    QHTMLPart *p = static_cast<QHTMLPart *>(static_cast<KParts::ReadOnlyPart *>(inPart));
    ...
    p->end();
    // we don't need to worry about child completion explicitly for QHTMLPart...
    // or do we?
    return true;
}

bool QHTMLPart::navigateChild(khtml::ChildFrame *child, const QUrl &url)
{
...
void QHTMLPart::connectToChildPart(khtml::ChildFrame *child, KParts::ReadOnlyPart *part,
                                   const QString &mimetype)
{
    // qCDebug(KHTML_LOG) << "we:" << this << "kid:" << child << part << mimetype;

    //AFA-fixed: check nullptr
    if (!part) {
        return;
    }
...
#if ENABLE(KJS)
        if (!qobject_cast<QHTMLPart *>(p) && child->m_jscript) {
...
//      qCDebug(KHTML_LOG) << "AH! NO FRAME!!!!!";
    ...
    if (qobject_cast<QHTMLPart *>(part)) {
        static_cast<QHTMLPart *>(part)->d->m_frame = child;
...
KParts::ReadOnlyPart *QHTMLPart::createPart(QWidget *parentWidget,
...
void QHTMLPart::submitFormAgain()
{
    disconnect(this, SIGNAL(completed()), this, SLOT(submitFormAgain()));
    if (d->m_doc && !d->m_doc->parsing() && d->m_submitForm) {
        QHTMLPart::submitForm(d->m_submitForm->submitAction, d->m_submitForm->submitUrl, d->m_submitForm->submitFormData, d->m_submitForm->target, d->m_submitForm->submitContentType, d->m_submitForm->submitBoundary);
    }
...
void QHTMLPart::submitFormProxy(const char *action, const QString &url, const QByteArray &formData, const QString &_target, const QString &contentType, const QString &boundary)
{
    submitForm(action, url, formData, _target, contentType, boundary);
}

void QHTMLPart::submitForm(const char *action, const QString &url, const QByteArray &formData, const QString &_target, const QString &contentType, const QString &boundary)
{
//    // qCDebug(KHTML_LOG) << this << "target=" << _target << "url=" << url;
//    if (d->m_formNotification == QHTMLPart::Only) {
//        emit formSubmitNotification(action, url, formData, _target, contentType, boundary);
//        return;
//    } else if (d->m_formNotification == QHTMLPart::Before) {
//        emit formSubmitNotification(action, url, formData, _target, contentType, boundary);
//    }
...
//            return;
//        }
//        d->m_submitForm = new QHTMLPartPrivate::SubmitForm;
...
void QHTMLPart::popupMenu(const QString &linkUrl)
{
//    QUrl popupURL;
//    QUrl linkKUrl;
//    KParts::OpenUrlArguments args;
...
//    // longer time than QHTMLPart. Deal with it.
...
void QHTMLPart::slotParentCompleted()
{
    //qCDebug(KHTML_LOG) << this;
...
void QHTMLPart::slotChildStarted(KIO::Job *job)
{
...
void QHTMLPart::slotChildCompleted()
{
    slotChildCompleted(false);
}

void QHTMLPart::slotChildCompleted(bool pendingAction)
{
    khtml::ChildFrame *child = frame(sender());
    ...
    if (child) {
        ...
        // dispatch load event. We don't do that for QHTMLPart's since their internal
        // load will be forwarded inside NodeImpl::dispatchWindowEvent
        if (!qobject_cast<QHTMLPart *>(child->m_part)) {
...
void QHTMLPart::slotChildDocCreated()
{
    ...
    if (QHTMLPart *htmlFrame = qobject_cast<QHTMLPart *>(sender())) {
...
void QHTMLPartPrivate::propagateInitialDomainAndBaseTo(QHTMLPart *kid)
{
...
void QHTMLPart::slotChildURLRequest(const QUrl &url, const KParts::OpenUrlArguments &args, const KParts::BrowserArguments &browserArgs)
{
    khtml::ChildFrame *child = frame(sender()->parent());
    QHTMLPart *callingHtmlPart = const_cast<QHTMLPart *>(dynamic_cast<const QHTMLPart *>(sender()->parent()));
...
void QHTMLPart::slotRequestFocus(KParts::ReadOnlyPart *)
{
    emit d->m_extension->requestFocus(this);
}

khtml::ChildFrame *QHTMLPart::frame(const QObject *obj)
{
...
bool QHTMLPart::checkFrameAccess(QHTMLPart *callingHtmlPart)
{
...
QHTMLPart *
QHTMLPart::findFrameParent(KParts::ReadOnlyPart *callingPart, const QString &f, khtml::ChildFrame **childFrame)
{
    return d->findFrameParent(callingPart, f, childFrame, false);
}

QHTMLPart *QHTMLPartPrivate::findFrameParent(KParts::ReadOnlyPart *callingPart,
        const QString &f, khtml::ChildFrame **childFrame, bool checkForNavigation)
{
...
#endif
    // Check access
    QHTMLPart *const callingHtmlPart = qobject_cast<QHTMLPart *>(callingPart);
    ...
    for (; it != end; ++it) {
        if (QHTMLPart *p = qobject_cast<QHTMLPart *>((*it)->m_part.data())) {
            QHTMLPart *const frameParent = p->d->findFrameParent(callingPart, f, childFrame, checkForNavigation);
            if (frameParent) {
                return frameParent;
            }
        }
...
QHTMLPart *QHTMLPartPrivate::top()
{
    QHTMLPart *t = q;
    while (t->parentPart()) {
        t = t->parentPart();
...
bool QHTMLPartPrivate::canNavigate(KParts::ReadOnlyPart *bCand)
{
    if (!bCand) { // No part here (e.g. invalid url), reuse that frame
        return true;
    }

    QHTMLPart *b = qobject_cast<QHTMLPart *>(bCand);
    ...
    for (QHTMLPart *anc = b->parentPart(); anc; anc = anc->parentPart()) {
...
QHTMLPart *QHTMLPart::findFrame(const QString &f)
{
    khtml::ChildFrame *childFrame;
    QHTMLPart *parentFrame = findFrameParent(this, f, &childFrame);
    if (parentFrame) {
        return qobject_cast<QHTMLPart *>(childFrame->m_part.data());
    }

    return nullptr;
}

KParts::ReadOnlyPart *QHTMLPart::findFramePart(const QString &f)
{
    khtml::ChildFrame *childFrame;
    return findFrameParent(this, f, &childFrame) ? childFrame->m_part.data() : nullptr;
}

KParts::ReadOnlyPart *QHTMLPart::currentFrame() const
{
    ...
    while (part && part->inherits("KHTMLPart") &&
            static_cast<QHTMLPart *>(part)->d->m_frames.count() > 0) {
        QHTMLPart *frameset = static_cast<QHTMLPart *>(part);
...
bool QHTMLPart::frameExists(const QString &frameName)
{
    FrameIt it = d->m_frames.find(frameName);
    if (it == d->m_frames.end()) {
        return false;
...
void QHTMLPartPrivate::renameFrameForContainer(DOM::HTMLPartContainerElementImpl *cont,
...
KJSProxy *QHTMLPart::framejScript(KParts::ReadOnlyPart *framePart)
{
    QHTMLPart *const kp = qobject_cast<QHTMLPart *>(framePart);
    if (kp) {
        return kp->jScript();
    }
...
QHTMLPart *QHTMLPart::parentPart()
{
    return qobject_cast<QHTMLPart *>(parent());
}

khtml::ChildFrame *QHTMLPart::recursiveFrameRequest(QHTMLPart *callingHtmlPart, const QUrl &url,
...
#endif
    khtml::ChildFrame *childFrame;
    QHTMLPart *childPart = findFrameParent(callingHtmlPart, browserArgs.frameName, &childFrame);
    if (childPart) {
        if (childPart == this) {
            return childFrame;
        }
...
void QHTMLPart::saveState(QDataStream &stream)
{
#if ENABLE(STATE)
#ifdef DEBUG_SAVESTATE
...
void QHTMLPart::restoreState(QDataStream &stream)
{
#if ENABLE(STATE)
    QUrl u;
...
//      qCDebug(KHTML_LOG) << *fNameIt << " ---- " << *fServiceTypeIt;
        ...
        if (false/*AFA !KHTMLPageCache::self()->isComplete(d->m_cacheId)*/) {
            d->m_restored = true;
            openUrl(u);
            d->m_restored = false;
        } else {
...
DOM::Node QHTMLPart::nodeUnderMouse() const
{
    return d->m_view->nodeUnderMouse();
}

DOM::Node QHTMLPart::nonSharedNodeUnderMouse() const
{
    return d->m_view->nonSharedNodeUnderMouse();
}

void QHTMLPart::emitSelectionChanged()
{
...
int QHTMLPart::zoomFactor() const
{
...
void QHTMLPart::slotIncZoom()
{
    zoomIn(zoomSizes, zoomSizeCount);
}

void QHTMLPart::slotDecZoom()
{
    zoomOut(zoomSizes, zoomSizeCount);
}

void QHTMLPart::slotIncZoomFast()
{
    zoomIn(fastZoomSizes, fastZoomSizeCount);
}

void QHTMLPart::slotDecZoomFast()
{
    zoomOut(fastZoomSizes, fastZoomSizeCount);
}

void QHTMLPart::zoomIn(const int stepping[], int count)
{
...
void QHTMLPart::zoomOut(const int stepping[], int count)
{
    int zoomFactor = d->m_zoomFactor;
...
void QHTMLPart::setZoomFactor(int percent)
{
...
void QHTMLPart::updateZoomFactor()
{
    if (d->m_view) {
...
void QHTMLPart::slotIncFontSize()
{
    incFontSize(zoomSizes, zoomSizeCount);
}

void QHTMLPart::slotDecFontSize()
{
    decFontSize(zoomSizes, zoomSizeCount);
}

void QHTMLPart::slotIncFontSizeFast()
{
    incFontSize(fastZoomSizes, fastZoomSizeCount);
}

void QHTMLPart::slotDecFontSizeFast()
{
    decFontSize(fastZoomSizes, fastZoomSizeCount);
}

void QHTMLPart::incFontSize(const int stepping[], int count)
{
...
void QHTMLPart::decFontSize(const int stepping[], int count)
{
    int zoomFactor = d->m_fontScaleFactor;
    if (zoomFactor > minZoom) {
        // find the entry nearest to the given zoomsizes
...
void QHTMLPart::setFontScaleFactor(int percent)
{
    if (percent < minZoom) {
        percent = minZoom;
    }
...
int QHTMLPart::fontScaleFactor() const
{
    return d->m_fontScaleFactor;
}

void QHTMLPart::slotZoomView(int delta)
{
    if (delta < 0) {
        slotIncZoom();
    } else {
...
void QHTMLPart::setStatusBarText(const QString &text, StatusBarPriority p)
{
    if (!d->m_statusMessagesEnabled) {
        return;
    }
    ...
    if (tobe.isEmpty()) {
        ...
        if (d->m_jobspeed) {
            //AFA tobe += i18n("(%1/s)",  KIO::convertSize(d->m_jobspeed));
        }
    }
    tobe = "<qt>" + tobe;
...
void QHTMLPart::setJSStatusBarText(const QString &text)
{
    setStatusBarText(text, BarOverrideText);
}

void QHTMLPart::setJSDefaultStatusBarText(const QString &text)
{
    setStatusBarText(text, BarDefaultText);
}

QString QHTMLPart::jsStatusBarText() const
{
    return d->m_statusBarText[BarOverrideText];
}

QString QHTMLPart::jsDefaultStatusBarText() const
{
    return d->m_statusBarText[BarDefaultText];
}

QString QHTMLPart::referrer() const
{
    return d->m_referrer;
}

QString QHTMLPart::pageReferrer() const
{
...
QString QHTMLPart::lastModified() const
{
...
void QHTMLPart::reparseConfiguration()
{
    QHTMLSettings *settings = QHTMLGlobal::defaultHTMLSettings();
    settings->init();
    ...
    delete d->m_settings;
    d->m_settings = new QHTMLSettings(*QHTMLGlobal::defaultHTMLSettings());
    ...
    if (QHTMLGlobal::defaultHTMLSettings()->isAdFilterEnabled()) {
        runAdFilter();
    }
}

QStringList QHTMLPart::frameNames() const
{
    QStringList res;
...
QList<KParts::ReadOnlyPart *> QHTMLPart::frames() const
{
    ...
    for (; it != end; ++it)
        if (!(*it)->m_bPreloaded && (*it)->m_part) // ### TODO: make sure that we always create an empty
            // QHTMLPart for frames so this never happens.
        {
...
bool QHTMLPart::openUrlInFrame(const QUrl &url, const KParts::OpenUrlArguments &args, const KParts::BrowserArguments &browserArgs)
{
...
void QHTMLPart::setDNDEnabled(bool b)
{
    d->m_bDnd = b;
}

bool QHTMLPart::dndEnabled() const
{
    return d->m_bDnd;
}

void QHTMLPart::customEvent(QEvent *event)
{
...
bool QHTMLPart::isPointInsideSelection(int x, int y)
{
...
void QHTMLPart::handleMousePressEventDoubleClick(khtml::MouseDoubleClickEvent *event)
{
...
void QHTMLPart::handleMousePressEventTripleClick(khtml::MouseDoubleClickEvent *event)
{
    QMouseEvent *mouse = event->qmouseEvent();
    DOM::Node innerNode = event->innerNode();
...
void QHTMLPart::handleMousePressEventSingleClick(khtml::MousePressEvent *event)
{
    QMouseEvent *mouse = event->qmouseEvent();
    DOM::Node innerNode = event->innerNode();
...
void QHTMLPart::khtmlMousePressEvent(khtml::MousePressEvent *event)
{
...
void QHTMLPart::khtmlMouseDoubleClickEvent(khtml::MouseDoubleClickEvent *event)
{
    QMouseEvent *_mouse = event->qmouseEvent();
    if (_mouse->button() == Qt::LeftButton) {
        d->m_bMousePressed = true;
...
bool QHTMLPart::isExtendingSelection() const
{
...
void QHTMLPart::extendSelectionTo(int x, int y, const DOM::Node &innerNode)
{
...
bool QHTMLPart::handleMouseMoveEventDrag(khtml::MouseMoveEvent *event)
{
    ...
    if ((d->m_bMousePressed &&
        ...
        if (url.isEmpty() && innerNodeImpl && innerNodeImpl->id() == ID_IMG) {
            ...
            //AFA pix = KIconLoader::global()->loadIcon("image-x-generic", KIconLoader::Desktop);
        } else {
            // Text or image link...
            u = completeURL(d->m_strSelectedURL);
            //AFA pix = KIO::pixmapForUrl(u, 0, KIconLoader::Desktop, KIconLoader::SizeMedium);
        }
        ...
        //AFA KUrlMimeData::setMetaData(metaDataMap, mimeData);
...
bool QHTMLPart::handleMouseMoveEventOver(khtml::MouseMoveEvent *event)
{
    ...
    if (url.length()) {
        ...
        // Image map
        if (!innerNode.isNull() && innerNode.handle()->id() == ID_IMG) {
...
void QHTMLPart::handleMouseMoveEventSelection(khtml::MouseMoveEvent *event)
{
...
void QHTMLPart::khtmlMouseMoveEvent(khtml::MouseMoveEvent *event)
{
...
void QHTMLPart::khtmlMouseReleaseEvent(khtml::MouseReleaseEvent *event)
{
    DOM::Node innerNode = event->innerNode();
    d->m_mousePressNode = DOM::Node();
    ...
    if (/*(d->m_guiProfile == BrowserViewGUI) && */(_mouse->button() == Qt::MidButton) && (event->url().isNull())) {
        ...
        if (d->m_bOpenMiddleClick) {
            ...
            //AFA p->d->m_extension->pasteRequest();
        }
    }
#endif
...
void QHTMLPart::khtmlDrawContentsEvent(khtml::DrawContentsEvent *)
{
}

#ifdef PART_USE_ENEVT
void QHTMLPart::guiActivateEvent(KParts::GUIActivateEvent *event)
{
    if (event->activated()) {
        emitSelectionChanged();
...
void QHTMLPart::slotSelectAll()
{
    KParts::ReadOnlyPart *part = currentFrame();
    if (part && part->inherits("KHTMLPart")) {
        static_cast<QHTMLPart *>(part)->selectAll();
    }
}

void QHTMLPart::startAutoScroll()
{
...
void QHTMLPart::runAdFilter()
{
    ...
    while (it.hasNext()) {
        ...
        if (obj->type() == khtml::CachedObject::Image) {
            ...
            image->m_wasBlocked = QHTMLGlobal::defaultHTMLSettings()->isAdFiltered(d->m_doc->completeURL(image->url().string()));
    ...
    if (QHTMLGlobal::defaultHTMLSettings()->isHideAdsEnabled()) {
        for (NodeImpl * nextNode, *node = d->m_doc; node; node = nextNode) {
            ...
            if (node->id() == ID_IMG ||
                ...
                if (QHTMLGlobal::defaultHTMLSettings()->isAdFiltered(d->m_doc->completeURL(static_cast<ElementImpl *>(node)->getAttribute(ATTR_SRC).trimSpaces().string()))) {
...
bool QHTMLPart::checkLinkSecurity(const QUrl &linkURL, const QString &message, const QString &button)
{
    bool linkAllowed = true;

    if (d->m_doc) {
        //AFA linkAllowed = KUrlAuthorized::authorizeUrlAction("redirect", url(), linkURL);
    }
...
void QHTMLPart::slotPartRemoved(KParts::Part *part)
{
...
void QHTMLPart::slotActiveFrameChanged(KParts::Part *part)
{
...
//    qCDebug(KHTML_LOG) << "d->m_activeFrame=" << d->m_activeFrame;
    ...
    //AFA d->m_extension->setExtensionProxy(KParts::BrowserExtension::childObject(d->m_activeFrame));
}

void QHTMLPart::setActiveNode(const DOM::Node &node)
{
    if (!d->m_doc || !d->m_view) {
        return;
    }
...
DOM::EventListener *QHTMLPart::createHTMLEventListener(QString code, QString name, NodeImpl *node, bool svg)
{
#if ENABLE(KJS)
    KJSProxy *proxy = jScript();
...
QHTMLPart *QHTMLPart::opener()
{
    return d->m_opener;
}

void QHTMLPart::setOpener(QHTMLPart *_opener)
{
    d->m_opener = _opener;
}

bool QHTMLPart::openedByJS()
{
    return d->m_openedByJS;
}

void QHTMLPart::setOpenedByJS(bool _openedByJS)
{
    d->m_openedByJS = _openedByJS;
}

void QHTMLPart::preloadStyleSheet(const QString &url, const QString &stylesheet)
{
    khtml::Cache::preloadStyleSheet(url, stylesheet);
}

void QHTMLPart::preloadScript(const QString &url, const QString &script)
{
    khtml::Cache::preloadScript(url, script);
}

long QHTMLPart::cacheId() const
{
    return d->m_cacheId;
}

bool QHTMLPart::restored() const
{
    return d->m_restored;
}

bool QHTMLPart::pluginPageQuestionAsked(const QString &mimetype) const
{
    // parentPart() should be const!
    QHTMLPart *parent = const_cast<QHTMLPart *>(this)->parentPart();
    if (parent) {
...
void QHTMLPart::setPluginPageQuestionAsked(const QString &mimetype)
{
    if (parentPart()) {
...
KEncodingDetector *QHTMLPart::createDecoder()
{
...
void QHTMLPart::emitCaretPositionChanged(const DOM::Position &pos)
{
...
void QHTMLPart::restoreScrollPosition()
{
...
void QHTMLPart::openWallet(DOM::HTMLFormElementImpl *form)
{
#ifndef KHTML_NO_WALLET
    QHTMLPart *p;
...
void QHTMLPart::saveToWallet(const QString &key, const QMap<QString, QString> &data)
{
#ifndef KHTML_NO_WALLET
    QHTMLPart *p;
...
void QHTMLPart::dequeueWallet(DOM::HTMLFormElementImpl *form)
{
#ifndef KHTML_NO_WALLET
    QHTMLPart *p;
...
void QHTMLPart::walletOpened(KWallet::Wallet *wallet)
{
#ifndef KHTML_NO_WALLET
    assert(!d->m_wallet);
    assert(d->m_wq);
...
KWallet::Wallet *QHTMLPart::wallet()
{
#ifndef KHTML_NO_WALLET
    QHTMLPart *p;
...
void QHTMLPart::slotWalletClosed()
{
#ifndef KHTML_NO_WALLET
    if (d->m_wallet) {
        d->m_wallet->deleteLater();
...
void QHTMLPart::launchWalletManager()
{
#ifndef KHTML_NO_WALLET
...
void QHTMLPart::removeStoredPasswordForm(QAction *action)
{
#ifndef KHTML_NO_WALLET
    assert(action);
    assert(d->m_wallet);
...
void QHTMLPart::addWalletFormKey(const QString &walletFormKey)
{
#ifndef KHTML_NO_WALLET
...
void QHTMLPart::delNonPasswordStorableSite()
{
#ifndef KHTML_NO_WALLET
    if (d->m_view) {
...
void QHTMLPart::saveLoginInformation(const QString &host, const QString &key, const QMap<QString, QString> &walletMap)
{
#ifndef KHTML_NO_WALLET
...
void QHTMLPart::slotToggleCaretMode()
{
    //AFA setCaretMode(d->m_paToggleCaretMode->isChecked());
}

void QHTMLPart::setFormNotification(QHTMLPart::FormNotification fn)
{
    d->m_formNotification = fn;
}

QHTMLPart::FormNotification QHTMLPart::formNotification() const
{
    return d->m_formNotification;
}

QUrl QHTMLPart::toplevelURL()
{
    QHTMLPart *part = this;
...
bool QHTMLPart::isModified() const
{
    if (!d->m_doc) {
        return false;
    }
...
void QHTMLPart::setDebugScript(bool enable)
{
...
void QHTMLPart::setSuppressedPopupIndicator(bool enable, QHTMLPart *originPart)
{
    if (parentPart()) {
...
void QHTMLPart::suppressedPopupMenu()
{
//    QMenu *m = new QMenu(nullptr);
...
void QHTMLPart::togglePopupPassivePopup()
{
...
void QHTMLPart::showSuppressedPopups()
{
#if ENABLE(KJS)
    foreach (QHTMLPart *part, d->m_suppressedPopupOriginParts) {
        if (part) {
...
QString QHTMLPart::defaultExtension() const
{
    if (!d->m_doc) {
        return ".html";
    }
...
bool QHTMLPart::inProgress() const
{
...
//AFA #include "moc_qhtmlpart_p.cpp"
#ifndef KHTML_NO_WALLET
